  ...
  <div class="flex">
    ...
    <div class="p-4 flex-1">
      ...
      <div class="bg-white rounded-2xl p-8 w-full max-w-xl mx-auto mt-10">
        <p class="text-gray-700 text-lg mb-7" style="color: #163891"> Completa el siguiente formulario para registrar un
          nuevo caso. Asegúrate de ingresar correctamente los datos antes de guardar.</p>

        <!-- Formulario -->
        <form @submit.prevent="guardarCaso">
          <!-- Campo Número de Pacientes -->
          <div class="mb-4">
            <label class="block text-gray-700 font-semibold mb-4 text-lg" for="numeroPacientes" style="color: #163891">
              Número de pacientes: </label>
            <input type="number" id="numeroPacientes" v-model="form.numeroPacientes" min="1"
          ...
          <!-- Campo Desglose -->
          <div class="mb-7">
            <label class="block text-gray-700 font-semibold mb-4 text-lg" for="desglose" style="color: #163891">
              Desglose:</label>
            <div class="relative">
              ...
              <ul v-if="isDesgloseDropdownOpen" class="absolute z-10 bg-white border rounded-lg mt-1 w-full shadow-lg">
                <li v-for="opcion in opcionesDesglose" :key="opcion" @click="selectDesglose(opcion)"
          ...
          <!-- Campo Doctor(a) -->
          <div class="mb-4">
            <label class="block text-gray-700 font-semibold mb-4 text-lg" for="doctor"
              style="color: #163891">Doctor(a):</label>
            <div class="relative mb-6">
              ...
              <ul v-if="isDoctorDropdownOpen" class="absolute z-10 bg-white border rounded-lg mt-1 w-full shadow-lg">
                <li v-for="doctor in doctores" :key="doctor.id" @click="selectDoctor(doctor)"
                  class="text-[#163891] hover:bg-gray-100 p-2 cursor-pointer">
                  {{ doctor.nombre }}
                </li>
              </ul>
...
export default {
  ...
  methods: {
    ...
    selectDoctor(opcion) {
      this.form.doctor = opcion.nombre;
      this.form.id_doctor = opcion.id;
    ...
    guardarCaso() {
      ...
      axios
        .post(`${this.$apiRoute}/registrar-caso`,
          {
            desglose: this.form.desglose,
            cantidad: this.form.numeroPacientes,
            id_doctor: this.form.id_doctor,
            id_reception: this.$user.reception
          },
          {
            headers: {
  ...
  mounted() {
    let token = localStorage.getItem('token');
    axios
      .get(`${this.$apiRoute}/mostrar-doctores/${this.$user.reception}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
...
select {
  -webkit-appearance: none;
  /* Oculta la flecha en Safari */
  -moz-appearance: none;
  /* Oculta la flecha en Firefox */
  appearance: none;
  /* Oculta la flecha en otros navegadores */
}
</style>
