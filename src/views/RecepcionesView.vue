  ...
  <div class="flex justify-center">
    ...
    <div class="p-4 flex-1">
      <div class="flex justify-between items-center mt-4 md:mt-6 mb-4">
        <h1 class="text-2xl md:text-3xl font-semibold ml-0 md:ml-12" style="color: #163891">Recepciones</h1>
      </div>

      <!-- Contenedor recepciones -->
      <div class="bg-white p-6 rounded-2xl max-w-4xl mx-auto mt-6 md:mt-10">
        <p class="text-[#163891] mb-5 mt-3 text-lg md:text-justify md:max-w-lg ml-4">Seleccione una recepción para ver
          las estadísticas de los pacientes atendidos y los doctores asociados.</p>

        <!-- Botón agregar recepcion -->
        <div class="flex justify-end mb-5 md:-mt-9">
          <button @click="$router.push('/agregar-recepcion')"
        ...
        <div v-for="(recepcion, index) in recepciones" :key="index"
          ...
          <div class="flex items-center space-x-4">
            <!-- Botón de editar -->
            <button @click="editarRecepcion(recepcion)" class="focus:outline-none">
              <svg class="w-6 h-6 transition-colors duration-200" :class="{
                'text-[#163891]': recepcionSeleccionada === index,
                'text-[#A3AED0]': recepcionSeleccionada !== index,
