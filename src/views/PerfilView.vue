      ...
      <div class="flex justify-center items-center mt-6 md:mt-10">
        <div class="bg-white rounded-2xl p-6 w-full max-w-lg">
          ...
          <div v-for="(usuario, index) in usuarios" :key="index"
            ...
            <div>
              <!-- Nombre del usuario -->
              <div class="text-[#163891] font-semibold text-lg"> {{ usuario.name }} </div>

              <!-- Rol del usuario -->
              <div class="text-[#A3AED0] text-sm">
                <div v-if="usuario.admin"> Administrador</div>
                <div v-else>Recepcionista</div>
              </div>
            </div>

            <!-- Botón de editar para cada usuario -->
            <button @click="editarUsuario(usuario)" class="focus:outline-none">
        ...
        <div class="bg-white rounded-2xl p-6 w-full max-w-lg">
          ...
          <div class="flex justify-center">
            ...
            <button @click="cerrarSesion"
              class="bg-[#2B3674] text-white font-semibold justify-center px-6 py-2 rounded-lg w-32">
              Aceptar
            </button>
          </div>
        </div>
      ...
      axios
        ...
        .catch(error => {
          if (error.response.status == 401) {
            alert("La sesion ha sido caducada.\nPorfavor vuelve a iniciar sesion.");
