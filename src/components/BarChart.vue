<template>
  <div class="chart-container w-[340px] md:w-[667px] h-[300px] bg-white rounded-2xl p-4 -mt-1 md:mt-4">
    <div class="header flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
      ...
      <div class="relative w-full sm:w-1/3 mt-2 sm:mt-0 flex sm:justify-end">
        ...
        >
          ...
          <svg class="h-4 w-4 text-[#163891] transition-transform" :class="{ 'rotate-180': isOpen }" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
            <path stroke-linecap="round" stroke-linejoin="round" d="M6 9l6 6 6-6" />
          </svg>
        </button>

        <div v-if="isOpen" class="absolute left-0 mt-2 w-52 h-auto rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 overflow-auto">
    ...
    <div class="flex justify-center h-[230px] md:h-[270px]">
      <Bar class="md:w-[500px]" id="my-chart-id" :options="chartOptions" :data="chartData" />
    </div>
  </div>
</template>
